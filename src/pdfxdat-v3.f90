...
real*8 dij,pdf,g,sigma,volume,dx,dy,dz,r,r_left, r_right
...
do t1=1,ntype
   do t2=1,ntype
      ...
      do idr=1,2000
         r=float(idr)/100.0
         r_left=float(idr-1)/100.0
         r_right=float(idr)/100.0
         pdf=0.0
         do isep=1,2000
            !g=gauss((r-(0.5+float(isep))/100.0)/(sigma))/sigma
            g=100.0*(erf((r_right-(0.5+float(isep))/100.0)/(sigma))-erf((r_left-(0.5+float(isep))/100.0)/(sigma)))/2
